...
class HammingNetwork:
    ...
    def train(self, patterns):
        # Нормализованные эталоны
        self.weights1 = np.array([p.flatten() for p in patterns])
    ...
    def predict(self, input_pattern, max_iter=100, epsilon=0.1):
        ...
        for t in range(self.num_classes):
            similarity = np.dot(input_vector, self.weights1[t]) / self.input_size
